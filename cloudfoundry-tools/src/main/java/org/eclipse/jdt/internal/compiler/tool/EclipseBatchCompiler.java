...
public class EclipseBatchCompiler extends EclipseCompilerImpl {
	...
	@Override
	protected void setPaths(ArrayList bootclasspaths, String sourcepathClasspathArg, ArrayList sourcepathClasspaths,
			ArrayList classpaths, ArrayList extdirsClasspaths, ArrayList endorsedDirClasspaths, String customEncoding) {
		...
		try {
			...
			super.setPaths(bootclasspaths, sourcepathClasspathArg, sourcepathClasspaths, classpaths, extdirsClasspaths,
					endorsedDirClasspaths, customEncoding);
		} finally {
		...
		if (this.fileManager instanceof ResourceJavaFileManager) {
			...
			Iterable<Folder> classPathFolders = resourceFolderJavaFileManager
					.getLocationFolders(StandardLocation.CLASS_PATH);
			if (classPathFolders != null) {
				...
				this.checkedClasspaths = checkedClasspathList.toArray(new FileSystem.Classpath[checkedClasspathList
						.size()]);
			}
		}
	}
